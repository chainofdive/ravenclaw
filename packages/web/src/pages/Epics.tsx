...
export function Epics() {
  ...
  useEffect(() => {
    api.listEpics().then(setEpics).catch((e) => setError(e.message));
  }, []);
  ...
  useEffect(() => {
    ...
    async function loadGraphData() {
      try {
        ...
        // Fetch all epic trees
        const treeResults = await Promise.allSettled(
          epics.map((e) => api.getEpicTree(e.id))
        );
        treeResults.forEach((result, i) => {
          if (result.status === 'fulfilled') {
            trees.set(epics[i].id, result.value);
          }
        });

        // Collect all issues from trees
        ...
        depResults.forEach((result, i) => {
          if (result.status === 'fulfilled') {
            dependencies.set(allIssues[i].id, result.value);
          }
        });

        if (!cancelled) {
          setGraphData({ epics, trees, dependencies });
  ...
  const toggleExpand = async (id: string) => {
    ...
    setExpanded(id);
    try {
      const data = await api.getEpicTree(id);
      setTree(data);
    } catch (e: any) {
      setError(e.message);
    }
    ...
    <div className="p-6 space-y-4">
          ...
          {epics.map((epic) => (
            <Card key={epic.id}>
              ...
              >
                ...
                {epic.description && (
                  <p className="text-xs text-slate-500 mt-1.5 ml-8">{epic.description}</p>
                )}
              </div>
      ...
      {view === 'graph' && (
        <>
          ...
          {!graphLoading && !graphData && epics.length === 0 && (
